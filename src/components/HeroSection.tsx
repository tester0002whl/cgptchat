import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { ArrowDown, MessageCircle, Shield, Zap, Users } from "lucide-react";
import { tools, toolCategories } from "@/data/tools";

interface HeroSectionProps {
  onScrollToPolicies: () => void;
  onOpenChat: () => void;
}

export function HeroSection({ onScrollToPolicies, onOpenChat }: HeroSectionProps) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 text-white">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-10"></div>
      
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32">
        <div className="text-center space-y-8">
          <div className="space-y-4">
            <Badge variant="secondary" className="bg-white/20 text-white border-white/30 hover:bg-white/30">
              🛡️ AI-Powered Cybersecurity Platform
            </Badge>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight">
              HackerGPT
              <span className="block text-blue-400">The Future of Security</span>
            </h1>
            <p className="max-w-3xl mx-auto text-xl sm:text-2xl text-white/90 leading-relaxed">
              Revolutionary AI-powered OSINT, penetration testing, and cybersecurity tools. 
              Automate complex security tasks with conversational intelligence.
            </p>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Button 
              size="lg"
              className="bg-blue-600 hover:bg-blue-700 text-white"
              onClick={onScrollToPolicies}
            >
              <ArrowDown className="w-5 h-5 mr-2" />
              Explore Tools
            </Button>
            <Button 
              size="lg"
              className="!bg-slate-700 !text-white !border-slate-600 hover:!bg-slate-600 hover:!text-white"
              onClick={onOpenChat}
            >
              <MessageCircle className="w-5 h-5 mr-2" />
              Chat with AI Assistant
            </Button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-4xl mx-auto pt-12">
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                <Shield className="w-8 h-8 text-blue-400" />
              </div>
              <div className="text-3xl font-bold text-blue-400">{tools.length}+</div>
              <div className="text-white/80">Security Tools</div>
            </div>
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                <Zap className="w-8 h-8 text-blue-400" />
              </div>
              <div className="text-3xl font-bold text-blue-400">{toolCategories.length}</div>
              <div className="text-white/80">Tool Categories</div>
            </div>
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                <Users className="w-8 h-8 text-blue-400" />
              </div>
              <div className="text-3xl font-bold text-blue-400">10K+</div>
              <div className="text-white/80">Security Professionals</div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}